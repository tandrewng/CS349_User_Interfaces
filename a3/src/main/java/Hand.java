...
public class Hand extends Appendage{
    ...
    private LowerArm parent = null;

    // Creates a rectangle based at the origin with the specified width and height
    public Hand(int x, int y, int width, int height, boolean isLeft, LowerArm p) {
        super();
        instance = ++instanceCount;
        parent = p;
        Image img;
        if (isLeft) {
            img = new Image("LeftHand.png");
        } else {
            img = new Image("RightHand.png");
        }
        this.setCenterX(x);
        this.setCenterY(y);
